...
$api->addEndpoint(Method::POST, ["type" => "addPanel", "show_from" => Type::date, "show_till" => Type::date, "fingerprint" => DataStructure::array, "panel_type" => array_map(fn ($e) => $e->value, PanelType::cases()), "content" => null], [], function () use ($user, $con) {
    if ($user == false) {
    ...
    $con->query("INSERT INTO panels (posted_by, `show_from`, `show_till`, `type`, `content`) VALUES (", [$user->id], ", ", [$_POST["show_from"]], ", ", [$_POST["show_till"]], ", ", [$panelType], ", ", [$_POST["content"]], ")");
